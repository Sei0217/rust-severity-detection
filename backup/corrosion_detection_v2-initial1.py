...
def main():
    ...
    try:
        while True:
            ...
            # Keep RGB for model, convert copy to BGR only for display
            frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) # For OpenCV display
            img_height, img_width = frame.shape[:2]
            
            # Preprocess using RGB frame
            input_image, scale, pad_top, pad_left = preprocess_image(frame, INPUT_SIZE) # Use RGB!
            ...
            # Post-process
            boxes, scores, class_ids = postprocess_detections(outputs, img_width, img_height, scale, pad_top, pad_left, CONFIDENCE_THRESHOLD)
            
            # Draw on BGR copy for display
